"""
Modal training script for Swedish Procurement Expert.
...
# Same base model as Riksbanken for consistency
...
app = modal.App("procurement-expert")

# Create a volume to persist model outputs
volume = modal.Volume.from_name("procurement-model-vol", create_if_missing=True)
VOLUME_PATH = "/vol"
...
def train(train_data: list[dict], val_data: list[dict] | None = None, resume_from_checkpoint: bool = False):
    """
    Fine-tune Mistral model on Swedish procurement Q&A.

    Args:
        train_data: List of training examples with 'messages' field.
    ...
    print("=" * 60)
    print("Swedish Procurement Expert - Training Pipeline")
    print("=" * 60)
...
def main(test_run: bool = True, resume: bool = False):
    ...
    test_prompt = "Vad är skillnaden mellan öppet och selektivt förfarande vid offentlig upphandling?"
    print(f"\nPrompt: {test_prompt}")
...
def compare_models(prompt: str = "Vad är skillnaden mellan öppet och selektivt förfarande vid offentlig upphandling?"):
    """
    ...
    print("\n" + "-" * 70)
    print("FINETUNED MODEL (with Procurement adapters):")
    print("-" * 70)
